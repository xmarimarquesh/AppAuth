...
export const LivingRoom = () => {
  ...
  const toggleSwitchLamp = async () => {
    ...
    await updateData('home/livingroom/light', newState);
  };

  const toggleSwitchAir = async () => {
    const newState = !isAirOn;
    setisAirOn(newState);
    await updateData('home/livingroom/air', newState);
  };
  ...
  useEffect(() => {
    const unsubscribeLamp = listenToData('home/livingroom/light', (data) => {
      setisLampOn(data);
    });

    const unsubscribeAir = listenToData('home/livingroom/air', (data) => {
      setisAirOn(data);
    });
